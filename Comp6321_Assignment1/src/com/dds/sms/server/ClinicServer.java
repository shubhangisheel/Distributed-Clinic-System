...
public class ClinicServer extends UnicastRemoteObject implements Runnable, ClinicServerInterface {
	...
	public ClinicServer(String location,int rmiRegisteryPort, int portNumber) throws RemoteException{
		...
		/*Fetching registry and binding object to it*/
		Registry r = LocateRegistry.getRegistry ( rmiRegisteryPort );
		r.rebind( location, this);
	...
	public int getCount(String recordType){
		...
		UDPClientServer objUDPCS = new UDPClientServer(this);
	...
	/*Function transferRecord function for Assignment 2*/
	public boolean transferRecord(String managerID,String recordID, String location){
		boolean flag = false;
		
		/*Checking recordID existence*/
		for(int i =0; i<list.size();i++){
			if(list.get(i).getRecordID().matches(recordID)){
				flag = true;				
				}
			else flag = false;
			}
		
		/*Creating object of another location: location*/
