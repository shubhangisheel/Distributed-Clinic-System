...
public class ManagerClient {
			...
			try {
				...
				if(serverObj.managerAuthentication(managerID)){
					...
					if ( choice  == 1 ) {
						...
						getTransferRecordMenu(serverObj);
					} else { 
	...
	public void getTransferRecordMenu(ClinicServerInterface serverObj){
		...
		try{
			...
			String managerID = r.readLine () ;
			System.out.println("Enter RecordID to be transferred: ");
			String recordID = r.readLine () ;
			...
			/*Passing user input to ClinicServer transferRecord*/
			boolean result;
			result = serverObj.transferRecord(managerID, recordID, location);
			if( result ){

				String message = "RecordID: "+ recordID + " is successfully transferred to destination location " + location +" at "+Calendar.getInstance().getTime() + "\n" ;
				writeLog(logFile, message);

				System.out.println("Thanks! RecordID is successfully transferred to destination location");
			}
			else{

				String message = "RecordID: "+ recordID + " is NOT successfully transferred to destination location " + location +" at "+Calendar.getInstance().getTime() + "\n" ;
				writeLog(logFile, message);
